...
export const actions: Actions = {
	default: async ({ locals, request }) => {
		...
		if (typeof userId !== 'string' || userId.length < 1 || userId.length > 31) return fail(400, { message: 'Invalid userId' });
		if (!/^[a-zA-Z0-9_-]+$/.test(userId)) return fail(400, { message: 'userId contains characters that cannot be used' });
		try {
			await locals.DB.update(users).set({ id: userId.toLowerCase() }).where(eq(users.id, session.user.userId));
		} catch (e: any) {
